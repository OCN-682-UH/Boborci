...
knitr: #this is to save figures automatically 
  opts_chunk:
    fig.path: "../Output/"----- This is to save figs to output-but won't render properly with this in YAML
...
Plot2Data$parameter.measured<- factor(Plot2Data$parameter.measured, levels = c("CCA Cover","Coral Cover","Algae Cover","Porites acuta Cover","Other Cover"))  # I want to keep the order so the colors match across figures
```
...
```{r}
#Need to pivot the data wider so all the parameters are their own columns
...
TableData%>%
  kbl(col.names=c("Site",  "% Nitrogen", "% CCA Cover", "% Algae Cover"))%>%   #Make column names pretty
   row_spec(6, bold=TRUE, color="black", background="lightpink")%>%    #Highlight row that we want to look at
  kable_material() 
```
